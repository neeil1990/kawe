...
if (isset($_GET['destId']))
	$destId = preg_replace("/[^a-zA-Z0-9_:\\[\\]]/", "", $_GET['destId']);
if ($destId != '')
	$reloadParams['destId'] = $destId;

$destSection = '';
if (isset($_GET['destSection']))
	$destSection = preg_replace("/[^a-zA-Z0-9_:]/", "", $_GET['destSection']);
if ($destSection != '')
	$reloadParams['destSection'] = $destSection;

$destDescrIblock = '';
if (isset($_GET['destDescrIblock']))
	$destDescrIblock = preg_replace("/[^a-zA-Z0-9_:]/", "", $_GET['destDescrIblock']);
if ($destDescrIblock != '')
	$reloadParams['destDescrIblock'] = $destDescrIblock;

$destDescrSection = '';
if (isset($_GET['destDescrSection']))
	$destDescrSection = preg_replace("/[^a-zA-Z0-9_:]/", "", $_GET['destDescrSection']);
if ($destDescrSection != '')
	$reloadParams['destDescrSection'] = $destDescrSection;
...
if ($tableId != '')
	$sTableID = 'tbl_asd_iblock_section_search_'.md5($tableId);
else
	$sTableID = 'tbl_asd_iblock_section_search_';
...
if($IBLOCK_ID > 0)
{
	...
	if($arIBlock)
	{
		$iblockName = GetMessage('IBLOCK_SECSEARCH_ENTITY_NAME', ['#NAME#' => $arIBlock['NAME']]);
...
$section_id = isset($find_section_section) && (string)$find_section_section != '' ? (int)$find_section_section: "";
	...
	if (!$m)
	{
		$row->AddActions(array(
			array(
				...
				"ACTION" => "javascript:SelEl(".$f_ID.", '".htmlspecialcharsbx($jsPath.htmlspecialcharsbx(CUtil::JSEscape($arRes["NAME"]), ENT_QUOTES)).$nameSeparator."')",
			),
		));
	}
}
...
{
	$contextMenu[] = [
		'TEXT' => GetMessage('IBLOCK_ROOT_SECTION_SELECT'),
		'TITLE' => GetMessage('IBLOCK_ROOT_SECTION_SELECT_TITLE'),
		'ONCLICK' => 'javascript:SelEl(0, \''.CUtil::JSEscape(GetMessage('IBLOCK_ROOT_SECTION_SELECT')).'\')'
	];
}
...
{
	var iblock, sect, title, iblockTitle;

	iblock = window.opener.document.getElementById('<?=$destId?>');
	if (iblock)
	{
		iblock.value = <?=$IBLOCK_ID; ?>;
	}
	sect = window.opener.document.getElementById('<?=$destSection?>');
	if (sect)
	{
		sect.value = id;
	}
	iblockTitle = window.opener.document.getElementById('<?=$destDescrIblock ?>');
	if (iblockTitle)
	{
		iblockTitle.innerText = '<?=CUtil::JSEscape($iblockName)?>';
	}
	title = window.opener.document.getElementById('<?=$destDescrSection?>');
	if (title)
	{
		...
		else if(e)
		{
			for(i=0;i<e.length;i++)
			{
				if (e[i].checked)
				{
					v = v + sep + e[i].value;
					sep = ',';
					n = n + sep2 + BX('name_'+e[i].value).innerHTML;
					sep2 = ', ';
				}
			}
		}
