...
/**
 * KDAPHPExcel_CachedObjectStorage_Wincache
 *
 * @category   KDAPHPExcel
 * @package	KDAPHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_CachedObjectStorage_Wincache extends KDAPHPExcel_CachedObjectStorage_CacheBase implements KDAPHPExcel_CachedObjectStorage_ICache {
		...
		if ($this->_currentCellIsDirty) {
			...
			if (wincache_ucache_exists($this->_cachePrefix.$this->_currentObjectID.'.cache')) {
				if (!wincache_ucache_set($this->_cachePrefix.$this->_currentObjectID.'.cache', $obj, $this->_cacheTime)) {
					$this->__destruct();
					throw new KDAPHPExcel_Exception('Failed to store cell '.$this->_currentObjectID.' in WinCache');
				}
			} else {
	...
	public function isDataSet($pCoord) {
		...
		if (parent::isDataSet($pCoord)) {
			...
			if ($success === false) {
				...
				throw new KDAPHPExcel_Exception('Cell entry '.$pCoord.' no longer exists in WinCache');
			}
			return true;
		}
		return false;
		...
		foreach($cacheList as $cellID) {
			if ($cellID != $this->_currentObjectID) {
				...
				if ($success === false) {
					...
					throw new KDAPHPExcel_Exception('Cell entry '.$cellID.' no longer exists in Wincache');
				}
				if (!wincache_ucache_add($newCachePrefix.$cellID.'.cache', $obj, $this->_cacheTime)) {
					$this->__destruct();
					throw new KDAPHPExcel_Exception('Failed to store cell '.$cellID.' in Wincache');
				}
			}
		}
