...
/**
 * KDAPHPExcel_CachedObjectStorage_Memcache
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_CachedObjectStorage_Memcache extends KDAPHPExcel_CachedObjectStorage_CacheBase implements KDAPHPExcel_CachedObjectStorage_ICache {
		...
		if ($this->_currentCellIsDirty) {
			...
			if (!$this->_memcache->replace($this->_cachePrefix.$this->_currentObjectID.'.cache',$obj,NULL,$this->_cacheTime)) {
				if (!$this->_memcache->add($this->_cachePrefix.$this->_currentObjectID.'.cache',$obj,NULL,$this->_cacheTime)) {
					$this->__destruct();
					throw new KDAPHPExcel_Exception('Failed to store cell '.$this->_currentObjectID.' in MemCache');
				}
			}
	...
	public function isDataSet($pCoord) {
		...
		if (parent::isDataSet($pCoord)) {
			...
			if ($success === false) {
				...
				throw new KDAPHPExcel_Exception('Cell entry '.$pCoord.' no longer exists in MemCache');
			}
			return true;
		}
		return false;
		...
		foreach($cacheList as $cellID) {
			if ($cellID != $this->_currentObjectID) {
				...
				if ($obj === false) {
					...
					throw new KDAPHPExcel_Exception('Cell entry '.$cellID.' no longer exists in MemCache');
				}
				if (!$this->_memcache->add($newCachePrefix.$cellID.'.cache',$obj,NULL,$this->_cacheTime)) {
					$this->__destruct();
					throw new KDAPHPExcel_Exception('Failed to store cell '.$cellID.' in MemCache');
				}
			}
		}
	 ...
	 *
	 * @param	KDAPHPExcel_Worksheet	$parent		The worksheet for this cell collection
	 * @param	array of mixed		$arguments	Additional initialisation arguments
	 */
	public function __construct(KDAPHPExcel_Worksheet $parent, $arguments) {
		...
		if (is_null($this->_cachePrefix)) {
			...
			if (!$this->_memcache->addServer($memcacheServer, $memcachePort, false, 50, 5, 5, true, array($this, 'failureCallback'))) {
				throw new KDAPHPExcel_Exception('Could not connect to MemCache server at '.$memcacheServer.':'.$memcachePort);
			}
	...
	public function failureCallback($host, $port) {
		throw new KDAPHPExcel_Exception('memcache '.$host.':'.$port.' failed');
	}
