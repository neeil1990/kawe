...
/**
 * KDAPHPExcel_CachedObjectStorage_SQLite
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_CachedObjectStorage_SQLite extends KDAPHPExcel_CachedObjectStorage_CacheBase implements KDAPHPExcel_CachedObjectStorage_ICache {
	...
	protected function _storeData() {
		if ($this->_currentCellIsDirty) {
			...
			if (!$this->_DBHandle->queryExec("INSERT OR REPLACE INTO kvp_".$this->_TableName." VALUES('".$this->_currentObjectID."','".sqlite_escape_string(serialize($this->_currentObject))."')"))
				throw new KDAPHPExcel_Exception(sqlite_error_string($this->_DBHandle->lastError()));
	...
	public function moveCell($fromAddress, $toAddress) {
		...
		if ($result === false)
			throw new KDAPHPExcel_Exception($this->_DBHandle->lastErrorMsg());
	 ...
	 */
	public function __construct(KDAPHPExcel_Worksheet $parent) {
