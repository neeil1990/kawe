...
/**
 * KDAPHPExcel_CachedObjectStorage_DiscISAM
 *
 * @category   KDAPHPExcel
 * @package    KDAPHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_CachedObjectStorage_DiscISAM extends KDAPHPExcel_CachedObjectStorage_CacheBase implements KDAPHPExcel_CachedObjectStorage_ICache {
	...
	public function copyCellCollection(KDAPHPExcel_Worksheet $parent) {
		...
		$newFileName = $this->_cacheDirectory.'/KDAPHPExcel.'.$baseUnique.'.cache';
	...
	public function __construct(KDAPHPExcel_Worksheet $parent, $arguments) {
		$this->_cacheDirectory	= ((isset($arguments['dir'])) && ($arguments['dir'] !== NULL))
									? $arguments['dir']
									: KDAPHPExcel_Shared_File::sys_get_temp_dir();

		parent::__construct($parent);
		if (is_null($this->_fileHandle)) {
			$baseUnique = $this->_getUniqueID();
			$this->_fileName = $this->_cacheDirectory.'/KDAPHPExcel.'.$baseUnique.'.cache';
			$this->_fileHandle = fopen($this->_fileName,'a+');
		}
