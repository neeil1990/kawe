 ...
 * @package		KDAPHPExcel_Chart_Renderer
...
require_once(KDAPHPExcel_Settings::getChartRendererPath().'/jpgraph.php');


/**
 * KDAPHPExcel_Chart_Renderer_jpgraph
 *
 * @category	KDAPHPExcel
 * @package		KDAPHPExcel_Chart_Renderer
 * @copyright	Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Chart_Renderer_jpgraph
{
	...
	private function _formatDataSetLabels($groupID, $datasetLabels, $labelCount, $rotation = '') {
		...
		foreach($datasetLabels as $i => $datasetLabel) {
			if (is_array($datasetLabel)) {
				...
				if (!is_null($datasetLabelFormatCode)) {
					$datasetLabels[$i] = KDAPHPExcel_Style_NumberFormat::toFormattedString($datasetLabel,$datasetLabelFormatCode);
				}
			}
			++$testCurrentIndex;
		}
	...
	/**
	 * Create a new KDAPHPExcel_Chart_Renderer_jpgraph
	 */
	public function __construct(KDAPHPExcel_Chart $chart)
	{
		$this->_graph	= null;
		$this->_chart	= $chart;
