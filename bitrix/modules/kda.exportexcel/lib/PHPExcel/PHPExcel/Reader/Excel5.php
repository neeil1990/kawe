 ...
 * @package    KDAPHPExcel_Reader_Excel5
 ...
 * @copyright	Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Reader_Excel5 extends KDAPHPExcel_Reader_Abstract implements KDAPHPExcel_Reader_IReader
{
	// ParseXL definitions
	 ...
	 * @var integer
	 */
	private $_loadingFileName;

	/**
	 * Current loading filename
	 *
	 * @var string
	 */
	private $_pos;
	...
	/**
	 * Create a new KDAPHPExcel_Reader_Excel5 instance
	 */
	...
	public function canRead($pFilename)
	{
		...
		try {
			...
			$ole = new KDAPHPExcel_Shared_OLERead();

			// get excel data
			$res = $ole->read($pFilename);
			return true;
		} catch (KDAPHPExcel_Exception $e) {
			return false;
		}
	}
	...
	public function listWorksheetNames($pFilename)
	{
		...
		$this->_dataSize = cp_strlen($this->_data);
			...
			while ($this->_pos <= $this->_dataSize - 4) {
				...
				switch ($code) {
					...
					case self::XLS_Type_LABEL:
						...
						$recordData = cp_substr($this->_data, $this->_pos + 4, $length);
	...
	public function load($pFilename)
	{
		...
		// Initialisations
		$this->_phpExcel = new KDAPHPExcel;
		...
		if (!$this->_readDataOnly && $this->_drawingGroupData) {
			$escherWorkbook = new KDAPHPExcel_Shared_Escher();
			$reader = new KDAPHPExcel_Reader_Excel5_Escher($escherWorkbook);
			$escherWorkbook = $reader->load($this->_drawingGroupData);

			// debug Escher stream
			//$debug = new Debug_Escher(new KDAPHPExcel_Shared_Escher());
		...
		foreach ($this->_sheets as $sheet) {
			...
			// add sheet to KDAPHPExcel object
			...
			if (!$this->_readDataOnly && $this->_drawingData) {
				$escherWorksheet = new KDAPHPExcel_Shared_Escher();
				$reader = new KDAPHPExcel_Reader_Excel5_Escher($escherWorksheet);
...
//				echo '<br />';
				...
				if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
					...
					list($startColumn, $startRow) = KDAPHPExcel_Cell::coordinateFromString($spContainer->getStartCoordinates());
					list($endColumn, $endRow) = KDAPHPExcel_Cell::coordinateFromString($spContainer->getEndCoordinates());
					
					if (($this->getReadFilter() !== NULL) && !$this->getReadFilter()->readCell($startColumn, $startRow, $this->_phpSheet->getTitle()) ) {
						continue;
					}
					...
					$width = KDAPHPExcel_Shared_Excel5::getDistanceX($this->_phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
					$height = KDAPHPExcel_Shared_Excel5::getDistanceY($this->_phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

					// calculate offsetX and offsetY of the shape
					$offsetX = $startOffsetX * KDAPHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, $startColumn) / 1024;
					$offsetY = $startOffsetY * KDAPHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $startRow) / 256;
...
//							echo 'Picture Object<br />';
							...
							if(isset($BSECollection[$BSEindex - 1]))
							{
								...
								if ($blip = $BSE->getBlip()) {
									...
									$drawing = new KDAPHPExcel_Worksheet_MemoryDrawing();
									...
									switch ($blipType) {
										case KDAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG:
											$drawing->setRenderingFunction(KDAPHPExcel_Worksheet_MemoryDrawing::RENDERING_JPEG);
											$drawing->setMimeType(KDAPHPExcel_Worksheet_MemoryDrawing::MIMETYPE_JPEG);
											break;

										case KDAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG:
											$drawing->setRenderingFunction(KDAPHPExcel_Worksheet_MemoryDrawing::RENDERING_PNG);
											$drawing->setMimeType(KDAPHPExcel_Worksheet_MemoryDrawing::MIMETYPE_PNG);
											break;
									}
			...
			if ($this->_version == self::XLS_BIFF8) {
				foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
					list($column, $row) = KDAPHPExcel_Cell::coordinateFromString($cell);
					if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($column, $row, $this->_phpSheet->getTitle()) ) {
						...
						$this->_phpSheet->getCell($cell)->setValueExplicit('=' . $formula, KDAPHPExcel_Cell_DataType::TYPE_FORMULA);
					}
				}
			}
...
//					echo '<br />';
			...
			if ($definedName['isBuiltInName']) {
						...
						if (count($explodes) == 2) {
							if ($docSheet = $this->_phpExcel->getSheetByName($explodes[0])) {
								...
								if (count($coordinateStrings) == 2) {
									list($firstColumn, $firstRow) = KDAPHPExcel_Cell::coordinateFromString($coordinateStrings[0]);
									list($lastColumn, $lastRow) = KDAPHPExcel_Cell::coordinateFromString($coordinateStrings[1]);
				...
				if (count($explodes) == 2) {
					if (($docSheet = $this->_phpExcel->getSheetByName($explodes[0])) ||
						...
						$this->_phpExcel->addNamedRange( new KDAPHPExcel_NamedRange((string)$definedName['name'], $docSheet, $extractedRange, $localOnly, $scope) );
					}
				} else {
					//	Named Value
	...
	{
		if($this->_loadingFileName==$pFilename) return;
		$this->_loadingFileName = $pFilename;
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x1E: // null-terminated string prepended by dword string length
					...
					$value = cp_substr($this->_summaryInformation, $secOffset + 8 + $offset, $byteLength);
					$value = KDAPHPExcel_Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
					$value = rtrim($value);
					break;

				case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-time
					$value = KDAPHPExcel_Shared_OLE::OLE2LocalDate(cp_substr($this->_summaryInformation, $secOffset + 4 + $offset, 8));
					break;
			...
			switch ($id) {
				case 0x01:	//	Code Page
					$codePage = KDAPHPExcel_Shared_CodePage::NumberToName($value);
					break;
...
//			echo 'Type is ',$type,', ';
			...
			switch ($type) {
				...
				case 0x1E:	//	null-terminated string prepended by dword string length
					...
					$value = cp_substr($this->_documentSummaryInformation, $secOffset + 8 + $offset, $byteLength);
					$value = KDAPHPExcel_Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
					$value = rtrim($value);
					break;
				...
				case 0x40:	//	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-Time
					$value = KDAPHPExcel_Shared_OLE::OLE2LocalDate(cp_substr($this->_documentSummaryInformation, $secOffset + 4 + $offset, 8));
					break;
...
//		echo '<b>Read Cell Annotation</b><br />';
		...
		$cellAddress = $this->_readBIFF8CellAddress(cp_substr($recordData, 0, 4));
		if ($this->_version == self::XLS_BIFF8) {
			$noteObjID = self::_GetInt2d($recordData, 6);
			$noteAuthor = self::_readUnicodeStringLong(cp_substr($recordData, 8));
...
//			echo 'Note Address=',$cellAddress,'<br />';
			...
			$noteText = trim(cp_substr($recordData, 6));
...
//													->setAuthor( $author )
	...
	private function _readTextObject()
	{
		...
		$this->_textObjects[$this->textObjRef] = array(
				'text'		=> cp_substr($text["recordData"],$text["spliceOffsets"][0]+1,$cchText),
				'format'	=> cp_substr($text["recordData"],$text["spliceOffsets"][1],$cbRuns),
				'alignment'	=> $grbitOpts,
				'rotation'	=> $rot
			 );
		...
		switch ($substreamType) {
			case self::XLS_WorkbookGlobals:
				...
				if (($version != self::XLS_BIFF8) && ($version != self::XLS_BIFF7)) {
					throw new KDAPHPExcel_Reader_Exception('Cannot read this Excel file. Version is too old.');
				}
...
//		$recordData = cp_substr($this->_data, $this->_pos + 4, $length);
		...
		throw new KDAPHPExcel_Reader_Exception('Cannot read encrypted file');
	}
	...
	private function _readCodepage()
	{
		...
		$this->_codepage = KDAPHPExcel_Shared_CodePage::NumberToName($codepage);
	}
	...
	private function _readDateMode()
	{
		...
		KDAPHPExcel_Shared_Date::setExcelCalendar(KDAPHPExcel_Shared_Date::CALENDAR_WINDOWS_1900);
		if (ord($recordData{0}) == 1) {
			KDAPHPExcel_Shared_Date::setExcelCalendar(KDAPHPExcel_Shared_Date::CALENDAR_MAC_1904);
		}
	}
	...
	private function _readFont()
	{
		...
		if (!$this->_readDataOnly) {
			$objFont = new KDAPHPExcel_Style_Font();
			...
			switch ($underlineType) {
				case 0x00:
					break; // no underline
				case 0x01:
					$objFont->setUnderline(KDAPHPExcel_Style_Font::UNDERLINE_SINGLE);
					break;
				case 0x02:
					$objFont->setUnderline(KDAPHPExcel_Style_Font::UNDERLINE_DOUBLE);
					break;
				case 0x21:
					$objFont->setUnderline(KDAPHPExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING);
					break;
				case 0x22:
					$objFont->setUnderline(KDAPHPExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING);
					break;
			}
			...
			if ($this->_version == self::XLS_BIFF8) {
				$string = self::_readUnicodeStringShort(cp_substr($recordData, 14));
			} else {
				$string = $this->_readByteStringShort(cp_substr($recordData, 14));
			}
	...
	private function _readFormat()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_version == self::XLS_BIFF8) {
				$string = self::_readUnicodeStringLong(cp_substr($recordData, 2));
			} else {
				// BIFF7
				$string = $this->_readByteStringShort(cp_substr($recordData, 2));
			}
	...
	private function _readXf()
	{
		...
		$objStyle = new KDAPHPExcel_Style();
		...
		if (!$this->_readDataOnly) {
			...
			} elseif (($code = KDAPHPExcel_Style_NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') {
			...
			$objStyle->getProtection()->setLocked($isLocked ?
				KDAPHPExcel_Style_Protection::PROTECTION_INHERIT : KDAPHPExcel_Style_Protection::PROTECTION_UNPROTECTED);

			// bit 1; mask 0x02; 1 = Formula is hidden
			$isHidden = (0x02 & $xfTypeProt) >> 1;
			$objStyle->getProtection()->setHidden($isHidden ?
				KDAPHPExcel_Style_Protection::PROTECTION_PROTECTED : KDAPHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
			...
			switch ($horAlign) {
				case 0:
					$objStyle->getAlignment()->setHorizontal(KDAPHPExcel_Style_Alignment::HORIZONTAL_GENERAL);
					break;
				case 1:
					$objStyle->getAlignment()->setHorizontal(KDAPHPExcel_Style_Alignment::HORIZONTAL_LEFT);
					break;
				case 2:
					$objStyle->getAlignment()->setHorizontal(KDAPHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					break;
				case 3:
					$objStyle->getAlignment()->setHorizontal(KDAPHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
					break;
				case 5:
					$objStyle->getAlignment()->setHorizontal(KDAPHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY);
					break;
				case 6:
					$objStyle->getAlignment()->setHorizontal(KDAPHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS);
					break;
			}
			...
			switch ($vertAlign) {
				case 0:
					$objStyle->getAlignment()->setVertical(KDAPHPExcel_Style_Alignment::VERTICAL_TOP);
					break;
				case 1:
					$objStyle->getAlignment()->setVertical(KDAPHPExcel_Style_Alignment::VERTICAL_CENTER);
					break;
				case 2:
					$objStyle->getAlignment()->setVertical(KDAPHPExcel_Style_Alignment::VERTICAL_BOTTOM);
					break;
				case 3:
					$objStyle->getAlignment()->setVertical(KDAPHPExcel_Style_Alignment::VERTICAL_JUSTIFY);
					break;
			}
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				// offset: 10; size: 4; Cell border lines and background area
					...
					if ($diagonalUp == false && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(KDAPHPExcel_Style_Borders::DIAGONAL_NONE);
					} elseif ($diagonalUp == true && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(KDAPHPExcel_Style_Borders::DIAGONAL_UP);
					} elseif ($diagonalUp == false && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(KDAPHPExcel_Style_Borders::DIAGONAL_DOWN);
					} elseif ($diagonalUp == true && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(KDAPHPExcel_Style_Borders::DIAGONAL_BOTH);
					}
	...
	private function _readXfExt()
	{
		...
		if (!$this->_readDataOnly) {
			...
			while ($offset < $length) {
				...
				// extension data
				$extData = cp_substr($recordData, $offset + 4, $cb);

				switch ($extType) {
					case 4:		// fill start color
						$xclfType  = self::_GetInt2d($extData, 0); // color type
						$xclrValue = cp_substr($extData, 4, 4); // color value (value based on color type)
	...
	private function _readStyle()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($isBuiltIn) {
				...
				// user-defined; not supported by KDAPHPExcel
			}
		}
	}
	...
	private function _readPalette()
	{
		...
		if (!$this->_readDataOnly) {
			...
			// list of RGB colors
			for ($i = 0; $i < $nm; ++$i) {
				$rgb = cp_substr($recordData, 2 + 4 * $i, 4);
	...
	private function _readSheet()
	{
		...
		switch (ord($recordData{4})) {
			case 0x00: $sheetState = KDAPHPExcel_Worksheet::SHEETSTATE_VISIBLE;    break;
			case 0x01: $sheetState = KDAPHPExcel_Worksheet::SHEETSTATE_HIDDEN;     break;
			case 0x02: $sheetState = KDAPHPExcel_Worksheet::SHEETSTATE_VERYHIDDEN; break;
			default: $sheetState = KDAPHPExcel_Worksheet::SHEETSTATE_VISIBLE;      break;
		}
		...
		if ($this->_version == self::XLS_BIFF8) {
			$string = self::_readUnicodeStringShort(cp_substr($recordData, 6));
			$rec_name = $string['value'];
		} elseif ($this->_version == self::XLS_BIFF7) {
			$string = $this->_readByteStringShort(cp_substr($recordData, 6));
			$rec_name = $string['value'];
		}
	...
	private function _readExternalBook()
	{
		...
		if (cp_strlen($recordData) > 4) {
			...
			$encodedUrlString = self::_readUnicodeStringLong(cp_substr($recordData, 2));
			$offset += $encodedUrlString['size'];

			// offset: var; size: var; list of $nm sheet names (Unicode strings, 16-bit length)
			$externalSheetNames = array();
			for ($i = 0; $i < $nm; ++$i) {
				$externalSheetNameString = self::_readUnicodeStringLong(cp_substr($recordData, $offset));
		...
		} elseif (cp_substr($recordData, 2, 2) == pack('CC', 0x01, 0x04)) {
			// internal reference
		...
		} elseif (cp_substr($recordData, 0, 4) == pack('vCC', 0x0001, 0x01, 0x3A)) {
			// add-in function
			// offset: 0; size: 2; 0x0001
			$this->_externalBooks[] = array(
				'type' => 'addInFunction',
			);
		} elseif (cp_substr($recordData, 0, 2) == pack('v', 0x0000)) {
			// DDE links, OLE links
	...
	private function _readExternName()
	{
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			// offset: 6; size: var
			$nameString = self::_readUnicodeStringShort(cp_substr($recordData, 6));

			// offset: var; size: var; formula data
			$offset = 6 + $nameString['size'];
			$formula = $this->_getFormulaFromStructure(cp_substr($recordData, $offset));
	...
	private function _readDefinedName()
	{
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			$string = self::_readUnicodeString(cp_substr($recordData, 14), $nlen);

			// offset: var; size: $flen; formula data
			$offset = 14 + $string['size'];
			$formulaStructure = pack('v', $flen) . cp_substr($recordData, $offset);
	...
	private function _readSst()
	{
		...
		for ($i = 0; $i < $nm; ++$i) {
			...
			if ($pos + $len <= $limitpos) {
				...
				$retstr = cp_substr($recordData, $pos, $len);
				$pos += $len;
				...
				$retstr = cp_substr($recordData, $pos, $limitpos - $pos);
	...
	private function _readHorizontalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($cf, $r, KDAPHPExcel_Worksheet::BREAK_ROW);
			}
		}
	}
	...
	private function _readVerticalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($c, $rf, KDAPHPExcel_Worksheet::BREAK_COLUMN);
			}
		}
	}
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (!$isNotInit) {
				...
				switch ($isPortrait) {
				case 0: $this->_phpSheet->getPageSetup()->setOrientation(KDAPHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE); break;
				case 1: $this->_phpSheet->getPageSetup()->setOrientation(KDAPHPExcel_Worksheet_PageSetup::ORIENTATION_PORTRAIT); break;
				}
			...
			$marginHeader = self::_extractNumber(cp_substr($recordData, 16, 8));
			$this->_phpSheet->getPageMargins()->setHeader($marginHeader);

			// offset: 24; size: 8; footer margin (IEEE 754 floating-point value)
			$marginFooter = self::_extractNumber(cp_substr($recordData, 24, 8));
	...
	private function _readRk()
	{
		...
		$columnString = KDAPHPExcel_Cell::stringFromColumnIndex($column);
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			// add cell
			$cell->setValueExplicit($numValue, KDAPHPExcel_Cell_DataType::TYPE_NUMERIC);
		}
	}
	...
	private function _readLabelSst()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				$richText = new KDAPHPExcel_RichText();
				$charPos = 0;
				$sstCount = count($this->_sst[$index]['fmtRuns']);
				for ($i = 0; $i <= $sstCount; ++$i) {
					if (isset($fmtRuns[$i])) {
						$text = KDAPHPExcel_Shared_String::Substring($this->_sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
						$charPos = $fmtRuns[$i]['charPos'];
					} else {
						$text = KDAPHPExcel_Shared_String::Substring($this->_sst[$index]['value'], $charPos, KDAPHPExcel_Shared_String::CountCharacters($this->_sst[$index]['value']));
					}

					if (KDAPHPExcel_Shared_String::CountCharacters($text) > 0) {
				...
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($richText, KDAPHPExcel_Cell_DataType::TYPE_STRING);
			} else {
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($this->_sst[$index]['value'], KDAPHPExcel_Cell_DataType::TYPE_STRING);
			}
	...
	private function _readMulRk()
	{
		...
		for ($i = 0; $i < $columns; ++$i) {
			$columnString = KDAPHPExcel_Cell::stringFromColumnIndex($colFirst + $i);
	...
	private function _readNumber()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			$numValue = self::_extractNumber(cp_substr($recordData, 6, 8));
	...
	private function _readFormula()
	{
		...
		$formulaStructure = cp_substr($recordData, 20);
		...
		if ($isPartOfSharedFormula) {
			...
			$this->_baseCell = KDAPHPExcel_Cell::stringFromColumnIndex($baseCol). ($baseRow + 1);
		}
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if ( (ord($recordData{6}) == 0)
				...
				$dataType = KDAPHPExcel_Cell_DataType::TYPE_STRING;
				...
				$dataType = KDAPHPExcel_Cell_DataType::TYPE_BOOL;
				...
				$dataType = KDAPHPExcel_Cell_DataType::TYPE_ERROR;
				...
				$dataType = KDAPHPExcel_Cell_DataType::TYPE_NULL;
				$value = '';

			} else {

				// forumla result is a number, first 14 bytes like _NUMBER record
				$dataType = KDAPHPExcel_Cell_DataType::TYPE_NUMERIC;
				$value = self::_extractNumber(cp_substr($recordData, 6, 8));
			...
			if (!$isPartOfSharedFormula) {
				...
				try {
					if ($this->_version != self::XLS_BIFF8) {
						throw new KDAPHPExcel_Reader_Exception('Not BIFF8. Can only read BIFF8 formulas');
					}
					$formula = $this->_getFormulaFromStructure($formulaStructure); // get formula in human language
					$cell->setValueExplicit('=' . $formula, KDAPHPExcel_Cell_DataType::TYPE_FORMULA);
	...
	private function _readSharedFmla()
	{
		...
		$cellRange = cp_substr($recordData, 0, 6);
		...
		$formula = cp_substr($recordData, 8);
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			switch ($isError) {
				case 0: // boolean
					...
					// add cell value
					$cell->setValueExplicit($value, KDAPHPExcel_Cell_DataType::TYPE_BOOL);
					break;
				...
				case 1: // error type
					...
					// add cell value
					$cell->setValueExplicit($value, KDAPHPExcel_Cell_DataType::TYPE_ERROR);
					break;
			}
	...
	private function _readMulBlank()
	{
		...
		if (!$this->_readDataOnly) {
			for ($i = 0; $i < $length / 2 - 3; ++$i) {
				$columnString = KDAPHPExcel_Cell::stringFromColumnIndex($fc + $i);
	...
	private function _readLabel()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if ($this->_version == self::XLS_BIFF8) {
				$string = self::_readUnicodeStringLong(cp_substr($recordData, 6));
				$value = $string['value'];
			} else {
				$string = $this->_readByteStringLong(cp_substr($recordData, 6));
				$value = $string['value'];
			}
			$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
			$cell->setValueExplicit($value, KDAPHPExcel_Cell_DataType::TYPE_STRING);
	...
	private function _readBlank()
	{
		...
		$columnString = KDAPHPExcel_Cell::stringFromColumnIndex($col);
...
//		echo '<br />';
	...
	private function _readWindow2()
	{
		...
		if ($this->_phpSheet->getSheetView()->getView() !== KDAPHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT) {
			//NOTE: this setting is inferior to page layout view(Excel2007-)
			$view = $isPageBreakPreview? KDAPHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW :
				KDAPHPExcel_Worksheet_SheetView::SHEETVIEW_NORMAL;
	...
	private function _readPageLayoutView(){
		...
		if ($fPageLayoutView === 1) {
			$this->_phpSheet->getSheetView()->setView(KDAPHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT);
	...
	private function _readPane()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_frozen) {
				// frozen panes
				$this->_phpSheet->freezePane(KDAPHPExcel_Cell::stringFromColumnIndex($px) . ($py + 1));
			} else {
				// unfrozen panes; split windows; not supported by KDAPHPExcel core
			}
		}
	}
	...
	private function _includeCellRangeFiltered($cellRangeAddress)
	{
		...
		if ($this->getReadFilter() !== NULL) {
			...
			$rangeBoundaries = KDAPHPExcel_Cell::getRangeBoundaries($cellRangeAddress);
			$rangeBoundaries[1][0]++;
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($hasDesc) {
				...
				$desc = self::_encodeUTF16(cp_substr($recordData, 36, 2 * ($dl - 1)), false);
				$offset += 4 + 2 * $dl;
			}
			if ($hasFrame) {
			...
			case 'URL':
				...
				$url = self::_encodeUTF16(cp_substr($recordData, $offset, $us - 2), false);
				$url .= $hasText ? '#' : '';
				$offset += $us;
				break;
			...
			case 'local':
				...
				$shortenedFilePath = cp_substr($recordData, $offset, $sl);
				$shortenedFilePath = self::_encodeUTF16($shortenedFilePath, true);
				$shortenedFilePath = cp_substr($shortenedFilePath, 0, -1); // remove trailing zero
				...
				if ($sz > 0) {
					...
					$extendedFilePath = cp_substr($recordData, $offset, $xl);
			...
			if ($hasText) {
				...
				$text = self::_encodeUTF16(cp_substr($recordData, $offset, 2 * ($tl - 1)), false);
				$url .= $text;
			}
			
			/*hyperlinks fix*/
			$arUrl = explode(chr(0), $url);
			$url = $arUrl[0];
			/*/hyperlinks fix*/

			// apply the hyperlink to all the relevant cells
			foreach (KDAPHPExcel_Cell::extractAllCellReferencesInRange($cellRange) as $coordinate) {
	...
	private function _readDataValidation()
	{
		...
		switch ($type) {
			case 0x00:	$type = KDAPHPExcel_Cell_DataValidation::TYPE_NONE;		break;
			case 0x01:	$type = KDAPHPExcel_Cell_DataValidation::TYPE_WHOLE;		break;
			case 0x02:	$type = KDAPHPExcel_Cell_DataValidation::TYPE_DECIMAL;		break;
			case 0x03:	$type = KDAPHPExcel_Cell_DataValidation::TYPE_LIST;		break;
			case 0x04:	$type = KDAPHPExcel_Cell_DataValidation::TYPE_DATE;		break;
			case 0x05:	$type = KDAPHPExcel_Cell_DataValidation::TYPE_TIME;		break;
			case 0x06:	$type = KDAPHPExcel_Cell_DataValidation::TYPE_TEXTLENGTH;	break;
			case 0x07:	$type = KDAPHPExcel_Cell_DataValidation::TYPE_CUSTOM;		break;
		}

		// bit: 4-6; mask: 0x00000070; error type
		$errorStyle = (0x00000070 & $options) >> 4;
		switch ($errorStyle) {
			case 0x00:	$errorStyle = KDAPHPExcel_Cell_DataValidation::STYLE_STOP;			break;
			case 0x01:	$errorStyle = KDAPHPExcel_Cell_DataValidation::STYLE_WARNING;		break;
			case 0x02:	$errorStyle = KDAPHPExcel_Cell_DataValidation::STYLE_INFORMATION;	break;
		}
		...
		switch ($operator) {
			case 0x00: $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_BETWEEN			;	break;
			case 0x01: $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_NOTBETWEEN		;	break;
			case 0x02: $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_EQUAL				;	break;
			case 0x03: $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_NOTEQUAL			;	break;
			case 0x04: $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_GREATERTHAN		;	break;
			case 0x05: $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_LESSTHAN			;	break;
			case 0x06: $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL;	break;
			case 0x07: $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL	;	break;
		}

		// offset: 4; size: var; title of the prompt box
		$offset = 4;
		$string = self::_readUnicodeStringLong(cp_substr($recordData, $offset));
		...
		$formula1 = cp_substr($recordData, $offset, $sz1);
		$formula1 = pack('v', $sz1) . $formula1; // prepend the length
		try {
			$formula1 = $this->_getFormulaFromStructure($formula1);

			// in list type validity, null characters are used as item separators
			if ($type == KDAPHPExcel_Cell_DataValidation::TYPE_LIST) {
		...
		$formula2 = cp_substr($recordData, $offset, $sz2);
		...
		$cellRangeAddressList = $this->_readBIFF8CellRangeAddressList(cp_substr($recordData, $offset));
		$cellRangeAddresses = $cellRangeAddressList['cellRangeAddresses'];

		foreach ($cellRangeAddresses as $cellRange) {
			$stRange = $this->_phpSheet->shrinkRangeToFit($cellRange);
			$stRange = KDAPHPExcel_Cell::extractAllCellReferencesInRange($stRange);
	...
	private function _readRangeProtection()
	{
		...
		if (!$this->_readDataOnly) {
			...
			for ($i = 0; $i < $cref; ++$i) {
				try {
					$cellRange = $this->_readBIFF8CellRangeAddressFixed(cp_substr($recordData, 27 + 8 * $i, 8));
				} catch (KDAPHPExcel_Exception $e) {
					return;
				}
			...
			$rgbFeat = cp_substr($recordData, $offset);
			$offset += 4;
...
//			var_dump($bcBitCount);
			...
			$rgbString = cp_substr($iData, 12);
			$rgbTriples = array();
			while (cp_strlen($rgbString) > 0) {
				$rgbTriples[] = unpack('Cb/Cg/Cr', $rgbString);
				$rgbString = cp_substr($rgbString, 3);
			}
			$x = 0;
			$y = 0;
			...
			$drawing = new KDAPHPExcel_Worksheet_Drawing();
	...
	private function _getSplicedRecordData()
	{
		...
		do {
			...
			$length = self::_GetInt2d($this->_data, $this->_pos + 2);
			$data .= cp_substr($this->_data, $this->_pos + 4, $length);
	...
	private function _getFormulaFromStructure($formulaStructure, $baseCell = 'A1')
	{
		...
		// offset: 2; size: sz
		$formulaData = cp_substr($formulaStructure, 2, $sz);
		...
		if (cp_strlen($formulaStructure) > 2 + $sz) {
			$additionalData = cp_substr($formulaStructure, 2 + $sz);
	...
	private function _getFormulaFromData($formulaData,  $additionalData = '', $baseCell = 'A1')
	{
		...
		while (cp_strlen($formulaData) > 0 and $token = $this->_getNextToken($formulaData, $baseCell)) {
			$tokens[] = $token;
			$formulaData = cp_substr($formulaData, $token['size']);
	...
	private function _createFormulaFromTokens($tokens, $additionalData)
	{
		...
		foreach ($tokens as $token) {
			...
			case 'tArray': // array constant
				...
				$additionalData = cp_substr($additionalData, $constantArray['size']); // bite of chunk of additional data
				unset($space0, $space1);
				break;
			case 'tMemArea':
				// bite off chunk of additional data
				$cellRangeAddressList = $this->_readBIFF8CellRangeAddressList($additionalData);
				$additionalData = cp_substr($additionalData, $cellRangeAddressList['size']);
	...
	private function _getNextToken($formulaData, $baseCell = 'A1')
	{
		...
		case 0x17:	//	string
			...
			$string = self::_readUnicodeStringShort(cp_substr($formulaData, 1));
			$size = 1 + $string['size'];
		...
		case 0x19:	//	Special attribute
			...
			case 0x41:
				...
				case 0x05:
					...
					break;
				default:
					throw new KDAPHPExcel_Reader_Exception('Unrecognized space type in tAttrSpace token');
					break;
				}
				...
				break;
			default:
				throw new KDAPHPExcel_Reader_Exception('Unrecognized attribute flag in tAttr token');
				break;
			}
			break;
		...
		case 0x1F:	//	number
			...
			$name = 'tNum';
			$size = 9;
			$data = self::_extractNumber(cp_substr($formulaData, 1));
		...
		case 0x61:
			...
			default:
				throw new KDAPHPExcel_Reader_Exception('Unrecognized function in formula');
				break;
			}
		...
		case 0x64:
			$name = 'tRef';
			$size = 5;
			$data = $this->_readBIFF8CellAddress(cp_substr($formulaData, 1, 4));
			break;
		...
		case 0x65:
			$name = 'tArea';
			$size = 9;
			$data = $this->_readBIFF8CellRangeAddress(cp_substr($formulaData, 1, 8));
			break;
		...
		case 0x66:
			...
			$size = 7 + $subSize;
			$data = $this->_getFormulaFromData(cp_substr($formulaData, 7, $subSize));
			break;
		...
		case 0x69:
			...
			$size = 3 + $subSize;
			$data = $this->_getFormulaFromData(cp_substr($formulaData, 3, $subSize));
			break;
		...
		case 0x6C:
			$name = 'tRefN';
			$size = 5;
			$data = $this->_readBIFF8CellAddressB(cp_substr($formulaData, 1, 4), $baseCell);
			break;
		...
		case 0x6D:
			$name = 'tAreaN';
			$size = 9;
			$data = $this->_readBIFF8CellRangeAddressB(cp_substr($formulaData, 1, 8), $baseCell);
			break;
		...
		case 0x7A:
			...
			try {
				...
				$cellAddress = $this->_readBIFF8CellAddress(cp_substr($formulaData, 3, 4));
		...
		case 0x7B:
			...
			try {
				...
				$cellRangeAddress = $this->_readBIFF8CellRangeAddress(cp_substr($formulaData, 3, 8));
		...
		default:
			throw new KDAPHPExcel_Reader_Exception('Unrecognized token ' . sprintf('%02X', $id) . ' in formula');
			break;
		}
	...
	private function _readBIFF8CellAddress($cellAddressStructure)
	{
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			$column = KDAPHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($cellAddressStructure, 2));
	...
	{
		list($baseCol, $baseRow) = KDAPHPExcel_Cell::coordinateFromString($baseCell);
		$baseCol = KDAPHPExcel_Cell::columnIndexFromString($baseCol) - 1;
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
			if (!(0x4000 & self::_GetInt2d($cellAddressStructure, 2))) {
				$column = KDAPHPExcel_Cell::stringFromColumnIndex($colIndex);
				$column = '$' . $column;
			} else {
				$colIndex = ($colIndex <= 127) ? $colIndex : $colIndex - 256;
				$column = KDAPHPExcel_Cell::stringFromColumnIndex($baseCol + $colIndex);
			}
	...
	private function _readBIFF5CellRangeAddressFixed($subData)
	{
		...
		// check values
		if ($fr > $lr || $fc > $lc) {
			throw new KDAPHPExcel_Reader_Exception('Not a cell range address');
		}

		// column index to letter
		$fc = KDAPHPExcel_Cell::stringFromColumnIndex($fc);
		$lc = KDAPHPExcel_Cell::stringFromColumnIndex($lc);
	...
	private function _readBIFF8CellRangeAddress($subData)
	{
		...
		// bit: 7-0; mask 0x00FF; column index
		$fc = KDAPHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 4));
		...
		// bit: 7-0; mask 0x00FF; column index
		$lc = KDAPHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 6));
	...
	private function _readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
	{
		...
		if (!(0x4000 & self::_GetInt2d($subData, 4))) {
			...
			$fc = KDAPHPExcel_Cell::stringFromColumnIndex($fcIndex);
			$fc = '$' . $fc;
		} else {
			// column offset
			$fcIndex = ($fcIndex <= 127) ? $fcIndex : $fcIndex - 256;
			$fc = KDAPHPExcel_Cell::stringFromColumnIndex($baseCol + $fcIndex);
		}
		...
		$lc = KDAPHPExcel_Cell::stringFromColumnIndex($baseCol + $lcIndex);

		// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
		if (!(0x4000 & self::_GetInt2d($subData, 6))) {
			// absolute column index
			$lc = KDAPHPExcel_Cell::stringFromColumnIndex($lcIndex);
			$lc = '$' . $lc;
		} else {
			// column offset
			$lcIndex = ($lcIndex <= 127) ? $lcIndex : $lcIndex - 256;
	...
	private function _readBIFF8CellRangeAddressList($subData)
	{
		...
		for ($i = 0; $i < $nm; ++$i) {
			$cellRangeAddresses[] = $this->_readBIFF8CellRangeAddressFixed(cp_substr($subData, $offset, 8));
			$offset += 8;
		}
	...
	private function _readBIFF5CellRangeAddressList($subData)
	{
		...
		for ($i = 0; $i < $nm; ++$i) {
			$cellRangeAddresses[] = $this->_readBIFF5CellRangeAddressFixed(cp_substr($subData, $offset, 6));
			$offset += 6;
		}
	 ...
	 * in which case an KDAPHPExcel_Reader_Exception is thrown
	 *
	 * @param int $index
	 * @return string|false
	 * @throws KDAPHPExcel_Reader_Exception
			...
			switch ($type) {
				case 'internal':
					...
					if ($this->_ref[$index]['firstSheetIndex'] == 0xFFFF or $this->_ref[$index]['lastSheetIndex'] == 0xFFFF) {
						throw new KDAPHPExcel_Reader_Exception('Deleted sheet reference');
					}
				...
				default:
					...
					throw new KDAPHPExcel_Reader_Exception('Excel5 reader only supports internal sheets in fomulas');
					break;
			}
		}
		return false;
	...
	private static function _readBIFF8ConstantArray($arrayData)
	{
		...
		$size = 3; // initialize
		$arrayData = cp_substr($arrayData, 3);
		...
		for ($r = 1; $r <= $nr + 1; ++$r) {
			...
			for ($c = 1; $c <= $nc + 1; ++$c) {
				...
				$arrayData = cp_substr($arrayData, $constant['size']);
				$size += $constant['size'];
			}
	...
	private static function _readBIFF8Constant($valueData)
	{
		...
		case 0x01: // number
			...
			$value = self::_extractNumber(cp_substr($valueData, 1, 8));
			$size = 9;
			break;
		case 0x02: // string value
			// offset: 1; size: var; Unicode string, 16-bit string length
			$string = self::_readUnicodeStringLong(cp_substr($valueData, 1));
			$value = '"' . $string['value'] . '"';
			$size = 1 + $string['size'];
			break;
		case 0x04: // boolean
	...
	private function _readByteStringShort($subData)
	{
		...
		$value = $this->_decodeCodepage(cp_substr($subData, 1, $ln));
	...
	private function _readByteStringLong($subData)
	{
		...
		$value = $this->_decodeCodepage(cp_substr($subData, 2));
	...
	private static function _readUnicodeStringShort($subData)
	{
		...
		$string = self::_readUnicodeString(cp_substr($subData, 1), $characterCount);
	...
	private static function _readUnicodeStringLong($subData)
	{
		...
		$string = self::_readUnicodeString(cp_substr($subData, 2), $characterCount);
	...
	private static function _readUnicodeString($subData, $characterCount)
	{
		...
		// needs to be fixed
		$value = self::_encodeUTF16(cp_substr($subData, 1, $isCompressed ? $characterCount : 2 * $characterCount), $isCompressed);
	...
	private static function _encodeUTF16($string, $compressed = '')
	{
		...
		return KDAPHPExcel_Shared_String::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
	}
	...
	{
		return KDAPHPExcel_Shared_String::ConvertEncoding($string, 'UTF-8', $this->_codepage);
	}
	...
	{
		switch ($index) {
			case 0x00: return KDAPHPExcel_Style_Border::BORDER_NONE;
			case 0x01: return KDAPHPExcel_Style_Border::BORDER_THIN;
			case 0x02: return KDAPHPExcel_Style_Border::BORDER_MEDIUM;
			case 0x03: return KDAPHPExcel_Style_Border::BORDER_DASHED;
			case 0x04: return KDAPHPExcel_Style_Border::BORDER_DOTTED;
			case 0x05: return KDAPHPExcel_Style_Border::BORDER_THICK;
			case 0x06: return KDAPHPExcel_Style_Border::BORDER_DOUBLE;
			case 0x07: return KDAPHPExcel_Style_Border::BORDER_HAIR;
			case 0x08: return KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHED;
			case 0x09: return KDAPHPExcel_Style_Border::BORDER_DASHDOT;
			case 0x0A: return KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOT;
			case 0x0B: return KDAPHPExcel_Style_Border::BORDER_DASHDOTDOT;
			case 0x0C: return KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT;
			case 0x0D: return KDAPHPExcel_Style_Border::BORDER_SLANTDASHDOT;
			default:   return KDAPHPExcel_Style_Border::BORDER_NONE;
		}
	}
	...
	{
		switch ($index) {
			case 0x00: return KDAPHPExcel_Style_Fill::FILL_NONE;
			case 0x01: return KDAPHPExcel_Style_Fill::FILL_SOLID;
			case 0x02: return KDAPHPExcel_Style_Fill::FILL_PATTERN_MEDIUMGRAY;
			case 0x03: return KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKGRAY;
			case 0x04: return KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTGRAY;
			case 0x05: return KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKHORIZONTAL;
			case 0x06: return KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKVERTICAL;
			case 0x07: return KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKDOWN;
			case 0x08: return KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKUP;
			case 0x09: return KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKGRID;
			case 0x0A: return KDAPHPExcel_Style_Fill::FILL_PATTERN_DARKTRELLIS;
			case 0x0B: return KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTHORIZONTAL;
			case 0x0C: return KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTVERTICAL;
			case 0x0D: return KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTDOWN;
			case 0x0E: return KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTUP;
			case 0x0F: return KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTGRID;
			case 0x10: return KDAPHPExcel_Style_Fill::FILL_PATTERN_LIGHTTRELLIS;
			case 0x11: return KDAPHPExcel_Style_Fill::FILL_PATTERN_GRAY125;
			case 0x12: return KDAPHPExcel_Style_Fill::FILL_PATTERN_GRAY0625;
			default:   return KDAPHPExcel_Style_Fill::FILL_NONE;
		}
	}
