...
if ($params['BILL_HEADER_SOTBIT'])
{
    ...
    $billNo_tmp = CSalePdf::prepareToPdf(
        $params['BILL_HEADER_SOTBIT'].' '.Loc::getMessage('SALE_HPS_BILL_SOTBIT_SELLER_TITLE', array('#PAYMENT_NUM#' => $order->getField('ACCOUNT_NUMBER'), '#PAYMENT_DATE#' => EditData($params["PAYMENT_DATE_INSERT"]), '#NUMBER#' => $params['NUMBER']))
    );
...
$name = \Bitrix\Main\Config\Option::get('sotbit.bill','FILE_NAME','N#ORDER_ID# from #DATE#');
if(!$name)
{
