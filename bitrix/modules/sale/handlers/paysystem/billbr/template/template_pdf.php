...
{
	$pdf->SetBackground(
		$params['BILLBR_BACKGROUND'],
		$params['BILLBR_BACKGROUND_STYLE']
	);
}
...
$margin = array(
	'top' => intval($params['BILLBR_MARGIN_TOP'] ?: 15) * 72/25.4,
	'right' => intval($params['BILLBR_MARGIN_RIGHT'] ?: 15) * 72/25.4,
	'bottom' => intval($params['BILLBR_MARGIN_BOTTOM'] ?: 15) * 72/25.4,
	'left' => intval($params['BILLBR_MARGIN_LEFT'] ?: 20) * 72/25.4
);
...
if ($params['BILLBR_PATH_TO_LOGO'])
{
	list($imageHeight, $imageWidth) = $pdf->GetImageSize($params['BILLBR_PATH_TO_LOGO']);

	$imgDpi = intval($params['BILLBR_LOGO_DPI']) ?: 96;
	$imgZoom = 96 / $imgDpi;
	...
	$pdf->Image($params['BILLBR_PATH_TO_LOGO'], $pdf->GetX(), $pdf->GetY(), -$imgDpi, -$imgDpi);
}
...
{
	$pdf->Ln();
	$text = CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_PHONE').".: %s", $params["SELLER_COMPANY_PHONE"]));
	$textWidth = $width - $logoWidth;
	while ($pdf->GetStringWidth($text))
	{
		list($string, $text) = $pdf->splitString($text, $textWidth);
...
if ($params['BILLBR_HEADER'])
{
	...
	$pdf->Cell(0, 15, CSalePdf::prepareToPdf($params['BILLBR_HEADER']), 0, 0, 'C');
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
}
...
{
	$pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_FOR')));
}
...
if ($params['BILLBR_PAYER_SHOW'] === 'Y')
{
	...
	$title = CSalePdf::prepareToPdf($params['BILLBR_HEADER'].' '.Loc::getMessage('SALE_HPS_BILLBR_NUMBER').': ');
...
if ($params['BILLBR_PAYER_SHOW'] === 'Y')
{
	...
	$title = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_DATE_INSERT').': ');
	$titleWidth = $pdf->GetStringWidth($title);
	$pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
	$pdf->Write(15, $title, 0, 0, 'R');
}
...
if ($params["DATE_PAY_BEFORE"])
{
	...
	$title = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_DATE_PAY_BEFORE').': ');
	$titleWidth = $pdf->GetStringWidth($title);
	$pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
	$pdf->Write(15, $title, 0, 0, 'R');
}
...
foreach ($columnList as $column)
{
	...
	{
		$arCols[$column] = array(
			'NAME' => CSalePdf::prepareToPdf($params['BILLBR_COLUMN_'.$column.'_TITLE']),
...
if ($params['BASKET_ITEMS'])
{
		...
		foreach ($arCols as $columnId => $col)
		{
			...
			switch ($columnId)
			{
				...
				case 'QUANTITY':
					...
					break;
				case 'MEASURE':
					$data = CSalePdf::prepareToPdf($basketItem["MEASURE_NAME"] ? $basketItem["MEASURE_NAME"] : Loc::getMessage('SALE_HPS_BILLBR_MEASURE'));
					$arCols[$columnId]['IS_DIGIT'] = true;
					break;
				case 'PRICE':
	...
	if ($params['BILLBR_TOTAL_SHOW'] == 'Y')
	{
		...
		if ($params['SUM'] - $sum > $eps)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_SUB_TOTAL').":");
		...
		if (!empty($vats))
		{
			...
			foreach ($vats as $vatRate => $vatSum)
			{
				...
				$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(sprintf(
					Loc::getMessage('SALE_HPS_BILLBR_TAX')." (%s%%):",
		...
		if ($params['SUM_PAID'])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_SUM_PAID').":");
		...
		if ($params['DISCOUNT_PRICE'])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_DISCOUNT').":");
		...
		$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_TOTAL').":");
...
for ($n = 1; $n <= $rowsCnt; $n++)
{
	...
	if ($params['BILLBR_COLUMN_NAME_SHOW'] == 'Y')
	{
...
{
	$pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_COMMENT')));
	$pdf->Ln();
	...
	if ($params["BILLBR_COMMENT1"])
	{
		$pdf->Write(15, HTMLToTxt(preg_replace(
			...
			CSalePdf::prepareToPdf($params["BILLBR_COMMENT1"])
		), '', array(), 0));
		$pdf->Ln();
		$pdf->Ln();
	}
	...
	if ($params["BILLBR_COMMENT2"])
	{
		$pdf->Write(15, HTMLToTxt(preg_replace(
			array('#</div>\s*<div[^>]*>#i', '#</?div>#i'), array('<br>', '<br>'),
			CSalePdf::prepareToPdf($params["BILLBR_COMMENT2"])
		), '', array(), 0));
		$pdf->Ln();
		$pdf->Ln();
	}
...
if ($params['BILLBR_PATH_TO_STAMP'])
{
	$filePath = $pdf->GetImagePath($params['BILLBR_PATH_TO_STAMP']);
	if ($filePath != '' && !$blank && \Bitrix\Main\IO\File::isFileExists($filePath))
	{
		list($stampHeight, $stampWidth) = $pdf->GetImageSize($params['BILLBR_PATH_TO_STAMP']);
...
if ($bankAccNo && $bankRouteNo && $bankSwift)
{
	...
	$pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_DETAIL')));
	$pdf->Ln();
	...
	if ($params["SELLER_COMPANY_NAME"])
	{
		$bankDetails .= CSalePdf::prepareToPdf(sprintf(
				Loc::getMessage('SALE_HPS_BILLBR_COMPANY_NAME').": %s\n",
				$params["SELLER_COMPANY_NAME"]
		));
	}

	$bankDetails .= CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK')." ".Loc::getMessage('SALE_HPS_BILLBR_NUMBER').": %s\n", $bankAccNo));
	...
	{
		$bankDetails .= CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_2').": ");
		if ($bank)
	...
	$bankDetails .= CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_ROUTE_NO').": %s\n", $bankRouteNo));
	$bankDetails .= CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SALE_HPS_BILLBR_COMPANY_BANK_SWIFT').": %s\n", $bankSwift));
