...
foreach ($columnList as $column)
{
	...
	{
		$arCols[$column] = array(
			'NAME' => CSalePdf::prepareToPdf($params['BILLLA_COLUMN_'.$column.'_TITLE']),
		...
		foreach ($arCols as $columnId => $col)
		{
			...
			switch ($columnId)
			{
				...
				case 'QUANTITY':
					...
					break;
				case 'MEASURE':
					$data = CSalePdf::prepareToPdf($basketItem["MEASURE_NAME"] ? $basketItem["MEASURE_NAME"] : Loc::getMessage('SALE_HPS_BILLLA_MEASURE'));
					$arCols[$columnId]['IS_DIGIT'] = true;
					break;
				case 'PRICE':
...
for ($n = 1; $n <= $rowsCnt; $n++)
{
	...
	if ($params['BILLLA_COLUMN_NAME_SHOW'] == 'Y')
	{
