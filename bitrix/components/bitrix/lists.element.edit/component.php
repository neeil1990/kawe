...
//Form submitted
if($_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid() && $arResult["IBLOCK_PERM"] >= "U")
{
	...
	elseif($arResult["ELEMENT_ID"] && isset($_POST["action"]) && $_POST["action"]==="delete" && $arResult["IBLOCK_PERM"] >= "W")
	{
		$obElement = new CIBlockElement;
		$obElement->Delete($arResult["ELEMENT_ID"]);
		LocalRedirect($arResult["~LIST_SECTION_URL"]);
	}
	elseif(isset($_POST["save"]) || isset($_POST["apply"]) && $arResult["IBLOCK_PERM"] >= "W")
	{
		$strError = "";
	...
	if($obList->is_field($FIELD_ID))
	{
		if($FIELD_ID == "ACTIVE_FROM")
		{
			...
			elseif($arField["DEFAULT_VALUE"] === "=now")
				$data[$FIELD_ID] = ConvertTimeStamp(false, "FULL");
			elseif($arField["DEFAULT_VALUE"] === "=today")
				$data[$FIELD_ID] = ConvertTimeStamp(false, "SHORT");
			else
				$data[$FIELD_ID] = "";
		}
		...
		foreach($value as $key1 => $value1)
		{
			if(is_array($value1))
			{
				foreach($value1 as $key2 => $value2)
					if(!is_array($value2))
						$value[$key1][$key2] = htmlspecialchars($value2);
			}
			else
			{
