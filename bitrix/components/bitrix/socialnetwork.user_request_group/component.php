	...
	else
	{
		...
		else
		{
			...
			elseif ($arResult["CurrentUserPerms"]["UserRole"] && $_REQUEST["EventType"] != "GroupRequest")
			{
				...
				if ($arResult["CurrentUserPerms"]["UserRole"] == SONET_ROLES_REQUEST)
				{
					...
					if ($arUserRequests = $dbUserRequests->GetNext())
					{
						...
						$arEventTmp["Urls"]["FriendAdd"] = htmlspecialchars($APPLICATION->GetCurUri("EventType=GroupRequest&eventID=".$arUserRequests["ID"]."&action=add&".bitrix_sessid_get().""));
						$arEventTmp["Urls"]["FriendReject"] = htmlspecialchars($APPLICATION->GetCurUri("EventType=GroupRequest&eventID=".$arUserRequests["ID"]."&action=reject&".bitrix_sessid_get().""));
			...
			else
			{
				...
				{
					if (!CSocNetUserToGroup::SendRequestToBeMember($GLOBALS["USER"]->GetID(), $arResult["Group"]["ID"], ""))
					{
						if ($e = $APPLICATION->GetException())
							$errorMessage .= $e->GetString();
					}
				...
				else
				{
					...
					if ($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["save"]) > 0 && check_bitrix_sessid())
					{
						...
						if (strlen($errorMessage) <= 0)
						{
							...
							if (!CSocNetUserToGroup::SendRequestToBeMember($GLOBALS["USER"]->GetID(), $arResult["Group"]["ID"], $_POST["MESSAGE"], $arResult["Urls"]["GroupRequests"]))
							{
								if ($e = $APPLICATION->GetException())
									$errorMessage .= $e->GetString();
							}
