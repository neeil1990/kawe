...
if(strlen($arParams["PATH_TO_USER_GROUPS"])<=0)
	$arParams["PATH_TO_USER_GROUPS"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_groups&".$arParams["USER_VAR"]."=#user_id#");

$arParams["PATH_TO_USER_GROUPS_ADD"] = trim($arParams["PATH_TO_USER_GROUPS_ADD"]);
if(strlen($arParams["PATH_TO_USER_GROUPS_ADD"])<=0)
	$arParams["PATH_TO_USER_GROUPS_ADD"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_groups_add&".$arParams["USER_VAR"]."=#user_id#");
...
if(strlen($arParams["PATH_TO_USER_BLOG"])<=0)
	$arParams["PATH_TO_USER_BLOG"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_blog&".$arParams["USER_VAR"]."=#user_id#");

$arParams["PATH_TO_USER_MICROBLOG"] = trim($arParams["PATH_TO_USER_MICROBLOG"]);
if(strlen($arParams["PATH_TO_USER_MICROBLOG"])<=0)
	$arParams["PATH_TO_USER_MICROBLOG"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_microblog&".$arParams["USER_VAR"]."=#user_id#");

$arParams["PATH_TO_USER_PHOTO"] = trim($arParams["PATH_TO_USER_PHOTO"]);
if(strlen($arParams["PATH_TO_USER_PHOTO"])<=0)
	$arParams["PATH_TO_USER_PHOTO"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_photo&".$arParams["USER_VAR"]."=#user_id#");

$arParams["PATH_TO_USER_FORUM"] = trim($arParams["PATH_TO_USER_FORUM"]);
if(strlen($arParams["PATH_TO_USER_FORUM"])<=0)
	$arParams["PATH_TO_USER_FORUM"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_forum&".$arParams["USER_VAR"]."=#user_id#");

$arParams["PATH_TO_USER_CALENDAR"] = trim($arParams["PATH_TO_USER_CALENDAR"]);
if(strlen($arParams["PATH_TO_USER_CALENDAR"])<=0)
	$arParams["PATH_TO_USER_CALENDAR"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_calendar&".$arParams["USER_VAR"]."=#user_id#");

$arParams["PATH_TO_USER_TASKS"] = trim($arParams["PATH_TO_USER_TASKS"]);
if(strlen($arParams["PATH_TO_USER_TASKS"])<=0)
	$arParams["PATH_TO_USER_TASKS"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_tasks&".$arParams["USER_VAR"]."=#user_id#");

$arParams["PATH_TO_USER_FILES"] = trim($arParams["PATH_TO_USER_FILES"]);
if(strlen($arParams["PATH_TO_USER_FILES"])<=0)
	$arParams["PATH_TO_USER_FILES"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_files&".$arParams["USER_VAR"]."=#user_id#");
...
if(strlen($arParams["PATH_TO_USER_CONTENT_SEARCH"])<=0)
	$arParams["PATH_TO_USER_CONTENT_SEARCH"] = htmlspecialchars($APPLICATION->GetCurPage()."?".$arParams["PAGE_VAR"]."=user_content_search&".$arParams["USER_VAR"]."=#user_id#");
...
else
{
	...
	else
	{
		...
		$arResult["CanView"]["files"] = (array_key_exists("files", $arResult["ActiveFeatures"]) && CSocNetFeaturesPerms::CanPerformOperation($GLOBALS["USER"]->GetID(), SONET_ENTITY_USER, $arResult["User"]["ID"], "files", "view", CSocNetUser::IsCurrentUserModuleAdmin()));
