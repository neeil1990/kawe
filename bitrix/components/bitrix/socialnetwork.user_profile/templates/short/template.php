				...
				<?if (!$arResult["CurrentUserPerms"]["IsCurrentUser"]):?>
					...
					<div class="bx-user-control">
						<ul>
							...
							<li class="bx-icon-action bx-icon-message"><nobr><a href="<?= $arResult["Urls"]["MessageChat"] ?>" onclick="window.open('<?= $arResult["Urls"]["MessageChat"] ?>', '', 'location=yes,status=no,scrollbars=yes,resizable=yes,width=700,height=550,top='+Math.floor((screen.height - 550)/2-14)+',left='+Math.floor((screen.width - 700)/2-5)); return false;"><?= GetMessage("SONET_C39_SEND_MESSAGE") ?></a></nobr></li>
							<?endif;?>
					...
					<?if ($arResult["UserFieldsMain"]["SHOW"] == "Y"):?>
						<?foreach ($arResult["UserFieldsMain"]["DATA"] as $fieldName => $arUserField):?>
							<?if (StrLen($arUserField["VALUE"]) > 0):?>
								<tr><td width="50%"><?= $arUserField["NAME"] ?>:</td>
								<td width="50%"><?= $arUserField["VALUE"] ?></td></tr>
							<?endif;?>
						<?endforeach;?>
					<?endif;?>
						...
						<?foreach ($arResult["UserPropertiesMain"]["DATA"] as $fieldName => $arUserField):?>
							<?if (is_array($arUserField["VALUE"]) && count($arUserField["VALUE"]) > 0 || !is_array($arUserField["VALUE"]) && StrLen($arUserField["VALUE"]) > 0):?>
								<tr>
									...
									<td width="50%">
										<?
										if (IsModuleInstalled('intranet') && (!IsModuleInstalled("extranet") || !CExtranet::IsExtranetSite()))
