...
$arParams['NAME_TEMPLATE'] = $arParams['NAME_TEMPLATE'] ? $arParams['NAME_TEMPLATE'] : GetMessage("SONET_C25_NAME_TEMPLATE_DEFAULT");
	...
	else
	{
			...
			) && check_bitrix_sessid())
		{
			...
			if (strlen($errorMessage) <= 0)
			{
				...
				{
					if (!CSocNetUserToGroup::TransferMember2Moderator($GLOBALS["USER"]->GetID(), $arResult["Group"]["ID"], $arIDs, CSocNetUser::IsCurrentUserModuleAdmin()))
					{
				...
				{
					if (!CSocNetUserToGroup::BanMember($GLOBALS["USER"]->GetID(), $arResult["Group"]["ID"], $arIDs, CSocNetUser::IsCurrentUserModuleAdmin()))
					{
						if ($e = $APPLICATION->GetException())
							$errorMessage .= $e->GetString();
					}
				...
				elseif (strlen($_POST["exclude"]) > 0 && $arResult["CurrentUserPerms"]["UserCanModifyGroup"])
				{
					foreach($arIDs as $relation_id)
					{
						...
						{
							if (!CSocNetLogEvents::DeleteByUserAndEntity($arRelation["USER_ID"], SONET_ENTITY_GROUP, $arRelation["GROUP_ID"]))
							{
								if ($e = $APPLICATION->GetException())
									$errorMessageTmp = $e->GetString();
								if (StrLen($errorMessageTmp) <= 0)
									$errorMessageTmp = GetMessage("SONET_C25_CANT_DELETE_INVITATION_LOG");
