		...
		else
		{
			...
			else
			{
				...
				if ($_SERVER["REQUEST_METHOD"]=="POST" && (strlen($_POST["resend"]) > 0 || strlen($_POST["reject"]) > 0) && check_bitrix_sessid())
				{
					...
					{
/*
						if (strlen($_POST["resend"]) > 0)
						{
							foreach($arIDs as $recipient_id)
							{
								...
								if (CSocNetUserToGroup::Delete($arRelation["ID"]))
								{
									...
									if (!CSocNetLogEvents::DeleteByUserAndEntity($arRelation["USER_ID"], SONET_ENTITY_GROUP, $arRelation["GROUP_ID"]))
									{
										...
										if (StrLen($errorMessageTmp) <= 0)
											$errorMessageTmp = GetMessage("SONET_C12_CANT_DELETE_INVITATION_LOG");
...
/*
							if (!CSocNetUserToGroup::RejectRequestToBeMember($GLOBALS["USER"]->GetID(), $arResult["Group"]["ID"], $arIDs))
							{
								if ($e = $APPLICATION->GetException())
									$errorMessage .= $e->GetString();
							}
