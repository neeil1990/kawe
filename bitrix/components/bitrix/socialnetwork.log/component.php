...
$arParams["AVATAR_SIZE"] = (isset($arParams["AVATAR_SIZE"]) ? intval($arParams["AVATAR_SIZE"]) : 30);
...
{
	$arParams["PATH_TO_USER_MICROBLOG"] = $parent->arResult["PATH_TO_USER_MICROBLOG"];
	$arParams["PATH_TO_USER_MICROBLOG_POST"] = $parent->arResult["PATH_TO_USER_MICROBLOG_POST"];
	$arParams["PATH_TO_GROUP_MICROBLOG"] = $parent->arResult["PATH_TO_GROUP_MICROBLOG"];
	if(strlen($arParams["PATH_TO_GROUP_MICROBLOG"]) <= 0)
		$arParams["PATH_TO_GROUP_MICROBLOG"] = $parent->arParams["PATH_TO_GROUP_MICROBLOG"];
	$arParams["BLOG_GROUP_ID"] = $parent->arResult["BLOG_GROUP_ID"];
}

$user_id = $USER->GetID();
if(IntVal($user_id) > 0)
{
	if(CSocNetFeatures::IsActiveFeature(SONET_ENTITY_USER, $user_id, "microblog"))
	{
		...
		while ($arEvents = $dbEvents->GetNext())
		{
			...
			foreach ($GLOBALS["arSocNetLogEvents"] as $event_id_tmp => $arEventTmp)
			{
				if (
					...
					&& in_array($arEvents["EVENT_ID"], $arEventTmp["FULL_SET"])
				)
				{
			...
			$arEvents["EVENT_ID_FULLSET"] = $event_id_fullset;
...
//			$timeFormated = $arDateTmp["HH"].':'.$arDateTmp["MI"].':'.$arDateTmp["SS"];
			...
			{
	//			get transport for log record
				if (array_key_exists($arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_".$arEvents["EVENT_ID"]."_N_N", $arCurrentUserSubscribe["TRANSPORT"]))
					$arEvents["FIELDS_FORMATTED"]["TRANSPORT"][] = $arCurrentUserSubscribe["TRANSPORT"][$arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_".$arEvents["EVENT_ID"]."_N_N"];
					
				if (array_key_exists($arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_all_N_N", $arCurrentUserSubscribe["TRANSPORT"]))
					$arEvents["FIELDS_FORMATTED"]["TRANSPORT"][] = $arCurrentUserSubscribe["TRANSPORT"][$arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_all_N_N"];

				if (CSocNetTools::HasLogEventCreatedBy($arEvents["EVENT_ID"]))
				{			
					if ($arEvents["EVENT_ID"])
					{
						if (array_key_exists("U_".$arEvents["USER_ID"]."_all_N_Y", $arCurrentUserSubscribe["TRANSPORT"]))
							$arEvents["FIELDS_FORMATTED"]["TRANSPORT"][] = $arCurrentUserSubscribe["TRANSPORT"]["U_".$arEvents["USER_ID"]."_all_N_Y"];
						elseif (array_key_exists("U_".$arEvents["USER_ID"]."_all_Y_Y", $arCurrentUserSubscribe["TRANSPORT"]))
							$arEvents["FIELDS_FORMATTED"]["TRANSPORT"][] = $arCurrentUserSubscribe["TRANSPORT"]["U_".$arEvents["USER_ID"]."_all_Y_Y"];
					}
				}

				if (
					!array_key_exists($arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_".$arEvents["EVENT_ID"]."_N_N", $arCurrentUserSubscribe["TRANSPORT"])
					&& !array_key_exists($arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_all_N_N", $arCurrentUserSubscribe["TRANSPORT"])
					)
				{
					if 
					(
						array_key_exists($arEvents["ENTITY_TYPE"], $arMyEntities)
						&& array_key_exists($arEvents["ENTITY_ID"], $arMyEntities[$arEvents["ENTITY_TYPE"]])
						&& array_key_exists($arEvents["ENTITY_TYPE"]."_0_".$arEvents["EVENT_ID"]."_Y_N", $arCurrentUserSubscribe["TRANSPORT"])
					)
						$arEvents["FIELDS_FORMATTED"]["TRANSPORT"][] = $arCurrentUserSubscribe["TRANSPORT"][$arEvents["ENTITY_TYPE"]."_0_".$arEvents["EVENT_ID"]."_Y_N"];
					elseif 
					(
						array_key_exists($arEvents["ENTITY_TYPE"], $arMyEntities)
						&& array_key_exists($arEvents["ENTITY_ID"], $arMyEntities[$arEvents["ENTITY_TYPE"]])
						&& array_key_exists($arEvents["ENTITY_TYPE"]."_0_all_Y_N", $arCurrentUserSubscribe["TRANSPORT"])
					)
						$arEvents["FIELDS_FORMATTED"]["TRANSPORT"][] = $arCurrentUserSubscribe["TRANSPORT"][$arEvents["ENTITY_TYPE"]."_0_all_Y_N"];
					elseif (array_key_exists($arEvents["ENTITY_TYPE"]."_0_".$arEvents["EVENT_ID"]."_N_N", $arCurrentUserSubscribe["TRANSPORT"]))
						$arEvents["FIELDS_FORMATTED"]["TRANSPORT"][] = $arCurrentUserSubscribe["TRANSPORT"][$arEvents["ENTITY_TYPE"]."_0_".$arEvents["EVENT_ID"]."_N_N"];
					elseif (array_key_exists($arEvents["ENTITY_TYPE"]."_0_all_N_N", $arCurrentUserSubscribe["TRANSPORT"]))
						$arEvents["FIELDS_FORMATTED"]["TRANSPORT"][] = $arCurrentUserSubscribe["TRANSPORT"][$arEvents["ENTITY_TYPE"]."_0_all_N_N"];
					else
						$arEvents["FIELDS_FORMATTED"]["TRANSPORT"][] = "N";
				}

				$arEvents["FIELDS_FORMATTED"]["TRANSPORT"] = array_unique($arEvents["FIELDS_FORMATTED"]["TRANSPORT"]);
				usort($arEvents["FIELDS_FORMATTED"]["TRANSPORT"], "__SLTransportSort");

	//			get visible for log record
				if (array_key_exists($arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_".$arEvents["EVENT_ID"]."_N_N", $arCurrentUserSubscribe["VISIBLE"]))
					$arEvents["FIELDS_FORMATTED"]["VISIBLE"] = $arCurrentUserSubscribe["VISIBLE"][$arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_".$arEvents["EVENT_ID"]."_N_N"];
				elseif (array_key_exists($arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_all_N_N", $arCurrentUserSubscribe["VISIBLE"]))
					$arEvents["FIELDS_FORMATTED"]["VISIBLE"] = $arCurrentUserSubscribe["VISIBLE"][$arEvents["ENTITY_TYPE"]."_".$arEvents["ENTITY_ID"]."_all_N_N"];
				elseif 
				(
					array_key_exists($arEvents["ENTITY_TYPE"], $arMyEntities)
					&& array_key_exists($arEvents["ENTITY_ID"], $arMyEntities[$arEvents["ENTITY_TYPE"]])
					&& array_key_exists($arEvents["ENTITY_TYPE"]."_0_".$arEvents["EVENT_ID"]."_Y_N", $arCurrentUserSubscribe["VISIBLE"])
				)
					$arEvents["FIELDS_FORMATTED"]["VISIBLE"] = $arCurrentUserSubscribe["VISIBLE"][$arEvents["ENTITY_TYPE"]."_0_".$arEvents["EVENT_ID"]."_Y_N"];
				elseif 
				(
					array_key_exists($arEvents["ENTITY_TYPE"], $arMyEntities)
					&& array_key_exists($arEvents["ENTITY_ID"], $arMyEntities[$arEvents["ENTITY_TYPE"]])
					&& array_key_exists($arEvents["ENTITY_TYPE"]."_0_all_Y_N", $arCurrentUserSubscribe["VISIBLE"])
				)
					$arEvents["FIELDS_FORMATTED"]["VISIBLE"] = $arCurrentUserSubscribe["VISIBLE"][$arEvents["ENTITY_TYPE"]."_0_all_Y_N"];
				elseif (array_key_exists($arEvents["ENTITY_TYPE"]."_0_".$arEvents["EVENT_ID"]."_N_N", $arCurrentUserSubscribe["VISIBLE"]))
					$arEvents["FIELDS_FORMATTED"]["VISIBLE"] = $arCurrentUserSubscribe["VISIBLE"][$arEvents["ENTITY_TYPE"]."_0_".$arEvents["EVENT_ID"]."_N_N"];
				elseif (array_key_exists($arEvents["ENTITY_TYPE"]."_0_all_N_N", $arCurrentUserSubscribe["VISIBLE"]))
					$arEvents["FIELDS_FORMATTED"]["VISIBLE"] = $arCurrentUserSubscribe["VISIBLE"][$arEvents["ENTITY_TYPE"]."_0_all_N_N"];
				else
					$arEvents["FIELDS_FORMATTED"]["VISIBLE"] = "Y";
			}
		...
		if ($arParams["SET_LOG_CACHE"] == "Y")
		{
			$max_id = CSocNetLog::GetCacheLastLogID();
			CSocNetLog::SetUserCache($GLOBALS["USER"]->GetID(), $max_id, $max_id, 0);
		}
	}
	else
