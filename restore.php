...
define('IP_LIMIT', '109.106.143.134');
define('INIT_TIMESTAMP', '1547471488');
...
$mArr_ru =  array(
			...
			"ERR_WRITE_ERROR" => "Ошибка записи в файл #FILE#<br>Проверьте свободное место на диске",
			"CNT_PARTS" => "частей",
...
$mArr_en = array(
			...
			"ERR_WRITE_ERROR" => "Write error on file #FILE#<br>Check your disk space",
			"CNT_PARTS" => "parts",
...
$mArr_de = array(
			...
			"ERR_WRITE_ERROR" => "Fehler beim Schreiben in #FILE#<br>Überprüfen Sie den verfügbaren Speicherplatz",
			"CNT_PARTS" => "Teile",
	...
	if (LANG == "ru")
	{
		...
		$MESS["ERROR_IP_CHANGED"] = "IP адрес клиента изменился, продолжение невозможно.";
		$MESS["ERROR_INIT_TIMESTAMP"] = "Время работы скрипта восстановления истекло. Загрузите новую версию.";
	...
	function extractFile()
	{
		...
		if ($header['type'] != 5) // пишем контент в файл
		{
			...
			while(++$this->ReadBlockCurrent <= $FileBlockCount && ($contents = $this->readBlock()))
			{
				...
				if ($ret === false || $ret === 0)
					return $this->Error(str_replace('#FILE#', $f, getMsg('ERR_WRITE_ERROR')));
